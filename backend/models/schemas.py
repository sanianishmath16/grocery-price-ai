...
# ---------------------------------------------------------------------------
# Image upload / vision feature
# ---------------------------------------------------------------------------

class ImageAnalyzeRequest(BaseModel):
    """
    Request body for POST /api/analyze-images.

    images_b64  — list of base64-encoded image strings (no data-URI prefix).
                  JPEG / PNG / WEBP, each resized client-side to ≤ 1024px.
    pincode     — delivery pincode (forwarded to price comparison).
    max_images  — enforced server-side (default 10).
    """
    images_b64: List[str] = Field(
        ...,
        min_length=1,
        max_length=10,
        description="Base64-encoded image data (no data URI prefix), max 10",
    )
    pincode: str = Field(..., pattern=r"^\d{6}$", description="6-digit delivery pincode")


class DetectedProductSchema(BaseModel):
    """A single grocery product identified from an image."""
    name: str
...
class ImageAnalyzeResponse(BaseModel):
    """
    Response from POST /api/analyze-images.

    vision_status   — one of: ok | no_products | not_configured | error
    detected        — products identified by the vision service
    compare_result  — price comparison (same as /api/compare) if products found
    error_message   — human-readable explanation when status != ok
    """
    vision_status: str
    detected: List[DetectedProductSchema] = Field(default_factory=list)
