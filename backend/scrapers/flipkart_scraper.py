"""
flipkart_scraper.py — Mock scraper for Flipkart Minutes.

Flipkart quirks: cheaper on staples, slightly pricier on dairy/snacks,
₹250 free-delivery threshold.
"""
...
class FlipkartScraper(BaseScraper):
    ...
    def app_url(self) -> str:
        return "https://www.flipkart.com/quick-commerce"
    ...
    async def search_product(self, name: str, pincode: str) -> List[ProductResult]:
        ...
        rng = random.Random(int(hashlib.md5((name + "flipkart").encode()).hexdigest(), 16))
        suffixes = ["", " - Flipkart Quick", " | Minutes"]
        results = [
            ProductResult(
                platform=self.app_name,
                product_id=f"fk_{abs(hash(name+str(i)))}",
                name=f"{name.title()}{rng.choice(suffixes)}",
                price=round(price * rng.uniform(0.96, 1.03), 2),
                unit="1 unit",
                in_stock=True,
            )
            for i in range(3)
