"""
instamart_scraper.py — Mock scraper for Swiggy Instamart.

Instamart quirks: pricier on staples, high free-delivery threshold (₹299).
"""
...
class InstamartScraper(BaseScraper):
    ...
    async def search_product(self, name: str, pincode: str) -> List[ProductResult]:
        ...
        rng = random.Random(int(hashlib.md5((name + "instamart").encode()).hexdigest(), 16))
        suffixes = ["", " - Instamart Pick", " (Swiggy Fresh)"]
        results = [
            ProductResult(
                platform=self.app_name,
                product_id=f"im_{abs(hash(name+str(i)))}",
                name=f"{name.title()}{rng.choice(suffixes)}",
                price=round(price * rng.uniform(0.99, 1.06), 2),
                unit="1 unit",
                in_stock=True,
            )
            for i in range(3)
